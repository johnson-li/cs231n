...
def rnn_step_backward(dnext_h, cache):
    ...
    raw = (1 - next_h * next_h) * dnext_h
    dx = np.dot(raw, Wx.T)
    dprev_h = np.dot(raw, Wh.T)
    dWx = np.dot(x.T, raw)
    dWh = np.dot(prev_h.T, raw)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        yi, cachei = rnn_step_forward(x[:, i, :], pre, Wx, Wh, b)
        pre = yi
        h[:, i, :] = yi
        cache.append(cachei)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    next_c = np.zeros((N, H))
    for i in range(T):
        next_h, next_c, ca = lstm_step_forward(x[:, i, :], next_h, next_c, Wx, Wh, b)
        cache.append(ca)
        h[:, i, :] = next_h
    cache.append(D)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[-1]
    dx = np.empty((N, T, D), dtype=dh.dtype)
    dWx = np.zeros((D, 4 * H), dtype=dh.dtype)
    dWh = np.zeros((H, 4 * H), dtype=dh.dtype)
    db = np.zeros((4 * H), dtype=dh.dtype)
    dc = np.zeros((N, H), dtype=dh.dtype)
    dh0 = np.zeros((N, H), dtype=dh.dtype)
    for i in range(T - 1, -1, -1):
        c = cache[i]
        dx[:, i, :], dh0, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, i, :] + dh0, dc, c)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
